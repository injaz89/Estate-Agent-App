...
// Sample test data with Sri Lankan locations and prices
...
describe('Search Functionality Tests', () => {
  ...
  it('should filter properties by price range', () => {
    const filters = { minPrice: 30000000, maxPrice: 50000000 };
    const results = searchProperties(testProperties, filters);
    
    expect(results).toHaveLength(1);
    expect(results[0].price).toBe(45000000);
    expect(results.every(p => p.price >= 30000000 && p.price <= 50000000)).toBe(true);
  });
  ...
  it('should filter properties by postcode area', () => {
    const filters = { postcodeArea: '00' };
    const results = searchProperties(testProperties, filters);
    
    expect(results).toHaveLength(1);
    expect(results[0].postcode).toBe('00700');
  });
...
describe('Multi-Criteria Search Tests', () => {
  ...
  it('should return empty array when no properties match criteria', () => {
    const filters = { minPrice: 100000000 };
    const results = searchProperties(testProperties, filters);
...
describe('Utility Functions Tests', () => {
  ...
  it('should format price correctly', () => {
    expect(formatPrice(45000000)).toBe('£45,000,000');
    expect(formatPrice(100000000)).toBe('£100,000,000');
    expect(formatPrice(9500000)).toBe('£9,500,000');
  });
  ...
  it('should correctly match single filter criteria', () => {
    ...
    expect(matchesFilter(property, 'minPrice', 40000000)).toBe(true);
    expect(matchesFilter(property, 'maxPrice', 50000000)).toBe(true);
  });
});
